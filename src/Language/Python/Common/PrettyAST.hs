...
instance {-# OVERLAPPABLE #-} Pretty (ImportRelative a) where
...
instance {-# OVERLAPPABLE #-} Pretty (Statement a) where
...
instance {-# OVERLAPPABLE #-} Pretty (RaiseExpr a) where
   pretty (RaiseV3 e) =
...
instance {-# OVERLAPPABLE #-} Pretty (YieldArg a) where
   pretty (YieldFrom e _annot) = text "from" <+> pretty e
   pretty (YieldExpr e) = pretty e

instance {-# OVERLAPPABLE #-} Pretty (DictMappingPair a) where
   pretty (DictMappingPair key val) = pretty key <> colon <+> pretty val

instance {-# OVERLAPPABLE #-} Pretty (Slice a) where
...
instance {-# OVERLAPPABLE #-} Pretty (Op a) where
   pretty (And {}) = text "and"
   pretty (Or {}) = text "or"
   pretty (Not {}) = text "not"
...
instance {-# OVERLAPPABLE #-} Pretty (AssignOp a) where
